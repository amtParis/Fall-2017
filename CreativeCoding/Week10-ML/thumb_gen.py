...
def boxParamsCenter(width, height):
    ...
    upper_x = math.floor(.5*(width - minSize))
    upper_y = math.floor(.5*(height - minSize))
    lower_x = upper_x + minSize
    lower_y = upper_y + minSize
    return upper_x, upper_y, lower_x, lower_y
    # if isLandscape(width, height):
    #     upper_x = int( math.floor( ( hW - hH ) ) )
    #     upper_y = 0
    #     lower_x = int( math.floor( hW + hH ) )
    #     lower_y = height
    #     return upper_x, upper_y, lower_x, lower_y
    # else:
    #     upper_x = 0
    #     upper_y = int(math.floor(hH - hW))
    #     lower_x = width
    #     lower_y = int(math.floor(hH + hW))
    #     return upper_x, upper_y, lower_x, lower_y
...
def makeThumb(img, size):
    ...
    """
    # cropped_img = ImageOps.fit(img, size, Image.ANTIALIAS)
    # cropit(img, size)
...
def saveThumbs(dstURL, thumbs):
	...
	for thumb in thumbs:
                try:
		    thumb.save(dstURL + '/' + 'thumb_' + str(i) + '.jpg', "JPEG")
		    i = i+1
